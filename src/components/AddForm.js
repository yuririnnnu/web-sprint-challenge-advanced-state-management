...
const AddForm = ({error, dispatch}) => {
    ...
    const handleSubmit = e => {            
        ...
        if (state.name === "" || state.position === "" || state.nickname === "") {
            ...
            dispatch(setError('You must provide name, positon and nickname'))
...
//Task List:
//1. Connect the error, setError and addSmurf actions to the AddForm component.
//2. Replace all instances of the error static variable with your error message state value. 
//3. Within the handleSubmit function, replace the static assignment to error with a call to the setError action. Test that an error is displayed when this validation code fails.
